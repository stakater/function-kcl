...
// replicas this is a rolling, self-healing recycle and removes the need for the
// manual `kubectl rollout restart` stopgap.
//
// This is a stability backstop; the durable fix for the leak itself is the
// compile cache (see kclrun.go). The two are complementary — the backstop bounds
// any residual growth regardless of the leak's exact internals.

// recycleConfig is read from the environment so it can be tuned per cluster via
// the same extraEnv mechanism already used for the us-2 override, without a
// rebuild.
